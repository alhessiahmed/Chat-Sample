...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    myPeer =
        widget.chat.createdBy == myID ? 'is_peer1_typing' : 'is_peer2_typing';
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            StreamBuilder<DocumentSnapshot<Chat>>(
                stream: FbFireStoreChatsController().fetchChat(widget.chat.id),
                builder: (context, snapshot) {
                  return snapshot.hasData
                      ? TypingIndicator(
                          showIndicator: myPeer == 'is_peer1_typing'
                              ? snapshot.data!.data()!.isPeer2Typing
                              : snapshot.data!.data()!.isPeer1Typing,
                        )
                      : Container();
                }),
            Divider(
